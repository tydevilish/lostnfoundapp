...
export default function Navigation() {
  ...
  // โหลดสถานะผู้ใช้จาก cookie ผ่าน API
  useEffect(() => {
    let alive = true;
    (async () => {
      try {
          ...
          <div className="hidden md:flex items-center gap-3">
            {checking ? (
              ...
              <div className="relative" ref={menuRef}>
                ...
                >
                  ...
                  <div className="hidden sm:flex flex-col items-start leading-tight mr-1">
                    <span className="text-blue-900 text-sm font-semibold truncate max-w-[160px]">
                ...
                >
                  ...
                  <Link
                    ...
                    role="menuitem"
                    className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-blue-900 hover:bg-blue-50 focus:bg-blue-50 focus:outline-none"
                  >
                  ...
                  <button
                    onClick={signOut}
                    role="menuitem"
                    className="mt-1 flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-red-600 hover:bg-red-50 focus:bg-red-50 focus:outline-none"
                  >
                    <svg width="18" height="18" viewBox="0 0 24 24">
                      <path
              ...
              <>
                <Link
                  href="/signin"
                  className="rounded-full border border-blue-900/20 text-blue-900 px-4 py-2 text-md font-medium hover:bg-blue-50 transition"
                >
                  เข้าสู่ระบบ
                </Link>
                <Link
                  href="/signup"
                  className="rounded-full px-5 py-2 text-md font-semibold text-white
            ...
            {checking ? (
              ...
              <>
                <div className="mt-3 flex items-center gap-3 rounded-xl border border-slate-200 p-3">
                  <div
                    className="h-10 w-10 rounded-full bg-gradient-to-br from-blue-900 to-blue-700 text-white
                                  flex items-center justify-center ring-2 ring-blue-200 text-sm font-bold"
                  >
                    {initials(user.firstName, user.lastName)}
                  </div>
                ...
                <Link
                  href="/profile"
                  onClick={() => setOpen(false)}
                  className="rounded-full px-4 py-2 text-md font-medium text-blue-900 text-center border border-blue-900/20 hover:bg-blue-50 transition"
                >
                  ข้อมูลส่วนตัว
                </Link>
                <button
                  onClick={signOut}
                  className="rounded-full px-4 py-2 text-md font-semibold text-red-600 text-center border border-red-200 hover:bg-red-50 transition"
                >
                  ออกจากระบบ
                </button>
              </>
              ...
              <div className="mt-2 flex items-center gap-2">
                <Link
                  href="/signin"
                  onClick={() => setOpen(false)}
                  className="flex-1 rounded-full border border-blue-900/20 text-blue-900 px-4 py-2 text-md font-medium text-center hover:bg-blue-50 transition"
                >
                  เข้าสู่ระบบ
                </Link>
                <Link
                  href="/signup"
                  onClick={() => setOpen(false)}
                  className="flex-1 rounded-full px-4 py-2 text-md font-semibold text-white text-center
                             bg-gradient-to-r from-blue-900 to-blue-700 shadow-sm hover:shadow transition"
                >
                  สมัครสมาชิก
                </Link>
              </div>
