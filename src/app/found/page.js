...
export default function Found() {
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/found?${params.toString()}`, { cache: "no-store" });
  ...
  useEffect(() => { setPage(1); }, [filters, items.length]);
  ...
  const onDropFiles = (e) => { e.preventDefault(); e.stopPropagation(); handleFiles(e.dataTransfer.files); };
  const removeFileAt = (i) => {
    setFiles((arr) => arr.filter((_, idx) => idx !== i));
    setPreviews((arr) => { URL.revokeObjectURL(arr[i]); return arr.filter((_, idx) => idx !== i); });
  };
  const makeCover = (i) => {
    if (i === 0) return;
    setFiles((arr) => { const cp = [...arr]; const [x] = cp.splice(i, 1); cp.unshift(x); return cp; });
    setPreviews((arr) => { const cp = [...arr]; const [x] = cp.splice(i, 1); cp.unshift(x); return cp; });
  };
  const setF = (e) => setForm((p) => ({ ...p, [e.target.id]: e.target.value }));

  const valid = useMemo(() => {
    const nameOK = !!form.name.trim();
    const catOK = !!form.category && (form.category !== "อื่น ๆ" || !!form.categoryOther.trim());
    const dtOK = !!form.datetime;
    const placeOK = !!form.place && (form.place !== "อื่น ๆ" || !!form.placeOther.trim());
    return nameOK && catOK && dtOK && placeOK;
  }, [form]);

  const submit = async () => {
    if (!valid) { setMsg({ type: "error", text: "กรอกข้อมูลที่จำเป็นให้ครบถ้วน" }); return; }
    setMsg(null);
    try {
      setSaving(true);
      ...
      fd.append("category", form.category === "อื่น ๆ" ? form.categoryOther.trim() : form.category);
      fd.append("description", form.description.trim());
      fd.append("datetime", form.datetime);
      fd.append("place", form.place === "อื่น ๆ" ? form.placeOther.trim() : form.place);
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok || data?.success === false) throw new Error(data?.message || "บันทึกไม่สำเร็จ");

      setForm({ category:"", categoryOther:"", name:"", description:"", datetime:"", place:"", placeOther:"", color:"", brand:"" });
  ...
  const mobileColsClass = mobileDensity === "x2" ? "grid-cols-2" : "grid-cols-1";
    ...
    <div className="min-h-[80vh] bg-gradient-to-b from-blue-100/50 to-white">
      ...
      <section className="relative bg-gradient-to-r from-blue-900 to-blue-700">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-10">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
            <div>
              <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-white">แจ้งพบของ</h1>
              <p className="text-blue-100 mt-1">
                คุณแจ้งพบของทั้งหมด <span className="font-semibold text-white">{mineCount}</span> รายการ
              </p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <main className="md:col-span-9">
            ...
            <div className="md:hidden mb-4 flex items-center justify-between">
              ...
              <div className="inline-flex items-center rounded-full border border-slate-300 overflow-hidden">
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-semibold ${mobileDensity==="x1" ? "bg-slate-400" : "bg-white text-slate-700"} `}
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-semibold border-l border-slate-300 ${mobileDensity==="x2" ? "bg-slate-400" : "bg-white text-slate-700"}`}
            ...
            {loadingList ? (
              <div className={`grid ${mobileColsClass} sm:grid-cols-2 xl:grid-cols-3 gap-5`}>
              ...
              <>
                ...
                <div className={`grid ${mobileColsClass} sm:grid-cols-2 xl:grid-cols-3 gap-5`}>
                  {pageItems.map((it) => <FoundCard key={it.id} item={it} />)}
                </div>
                  ...
                  <div className="mt-6 flex items-center justify-between gap-3">
                    ...
                    <div className="inline-flex items-center gap-2">
                      <button
                        ...
                        disabled={page <= 1}
                        className={`rounded-full px-3 py-1.5 text-sm border ${page<=1 ? "opacity-50 cursor-not-allowed" : "hover:bg-slate-50"} border-slate-300 text-slate-700`}
                      >
                        ← ก่อนหน้า
                      </button>
      ...
      {openAdd && (
        <div className="fixed inset-0 z-[70] flex" role="dialog" aria-modal="true"
             onMouseDown={(e) => { if (e.target === e.currentTarget) setOpenAdd(false); }}>
          <div className="absolute inset-0 bg-black/50 backdrop-blur-[1px]" />
          <div className="relative mx-auto w-full h-[100dvh] bg-white
            ...
            <div className="sticky top-0 z-10 bg-white/95 backdrop-blur border-b border-slate-100 rounded-full">
              <div className="px-4 sm:px-6 py-3 flex items-center justify-between rounded-full">
                <div>
                  <h3 className="text-base sm:text-lg font-bold text-blue-900">แจ้งพบของ</h3>
                  <p className="text-xs sm:text-sm text-slate-600">กรอกรายละเอียดให้ครบถ้วนเพื่อให้เจ้าของติดต่อกลับได้ง่ายขึ้น</p>
                </div>
                <button onClick={() => setOpenAdd(false)} className="rounded-full p-2 text-slate-600 hover:bg-slate-100" aria-label="Close">✕</button>
              </div>
              {msg && (
                <div className={[
            ...
            <div className="px-4 sm:px-6 py-5 overflow-y-auto grow">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="ประเภทสิ่งของ" required>
                  <select id="category" ref={firstFieldRef} value={form.category} onChange={setF}
                ...
                {form.category === "อื่น ๆ" && (
                  <Field label="ระบุประเภทอื่น ๆ" required>
                    <input id="categoryOther" value={form.categoryOther} onChange={setF} placeholder="เช่น ถุงผ้า, พาวเวอร์แบงก์"
                ...
                <Field label="ชื่อสิ่งของ" required>
                  <input id="name" value={form.name} onChange={setF} placeholder="เช่น กระเป๋าสตางค์สีน้ำตาล"
                         className="w-full text-black rounded-xl border border-slate-300 px-4 py-2.5 outline-none focus:border-blue-900 focus:ring-2 focus:ring-blue-200" />
                </Field>

                <Field label="วัน–เวลาที่พบเจอ" required>
                  <input id="datetime" type="datetime-local" value={form.datetime} onChange={setF}
                         className="w-full text-black rounded-xl border border-slate-300 px-4 py-2.5 outline-none focus:border-blue-900 focus:ring-2 focus:ring-blue-200" />
                </Field>

                <Field label="สถานที่" required>
                  <select id="place" value={form.place} onChange={setF}
                          className="w-full text-black bg-white rounded-xl border border-slate-300 px-4 py-2.5 outline-none focus:border-blue-900 focus:ring-2 focus:ring-blue-200">
                ...
                {form.place === "อื่น ๆ" && (
                  <Field label="ระบุสถานที่อื่น ๆ" required>
                    <input id="placeOther" value={form.placeOther} onChange={setF} placeholder="เช่น ร้านกาแฟหน้ามหาวิทยาลัย"
                           className="w-full text-black rounded-xl border border-slate-300 px-4 py-2.5 outline-none focus:border-blue-900 focus:ring-2 focus:ring-blue-200" />
                  </Field>
                )}

                <Field label="สี/ลักษณะเด่น">
                  <input id="color" value={form.color} onChange={setF} placeholder="เช่น น้ำตาลเข้ม มีรอยขีดเล็ก ๆ"
                         className="w-full text-black rounded-xl border border-slate-300 px-4 py-2.5 outline-none focus:border-blue-900 focus:ring-2 focus:ring-blue-200" />
                </Field>
                <Field label="ยี่ห้อ/รุ่น (ถ้ามี)">
                  <input id="brand" value={form.brand} onChange={setF} placeholder="เช่น Apple iPhone 13"
                         className="w-full text-black rounded-xl border border-slate-300 px-4 py-2.5 outline-none focus:border-blue-900 focus:ring-2 focus:ring-blue-200" />
                </Field>

                <Field label="รายละเอียดเพิ่มเติม" full>
                  <textarea id="description" rows={4} value={form.description} onChange={setF} placeholder="เช่น เก็บไว้ที่ประชาสัมพันธ์อาคาร X"
                            className="w-full text-black rounded-xl border border-slate-300 px-4 py-2.5 outline-none focus:border-blue-900 focus:ring-2 focus:ring-blue-200" />
                </Field>
                ...
                <Field label="รูปภาพสิ่งของ" full>
                  ...
                  >
                    <div className="flex flex-col items-center gap-2">
                      <div className="h-12 w-12 rounded-full bg-blue-50 grid place-items-center text-blue-700">📷</div>
                      ...
                      <div className="text-[11px] text-slate-500">รองรับ JPG/PNG/WEBP หลายไฟล์ (รูปแรกจะเป็นหน้าปก)</div>
                    </div>
                  </div>
                    ...
                    <div className="mt-3 grid grid-cols-3 sm:grid-cols-4 gap-3">
                      {previews.map((src, i) => (
                        <div key={i} className="relative group rounded-lg overflow-hidden">
                          <img src={src} alt={`preview-${i}`} className="h-28 sm:h-32 w-full object-cover" />
                          ...
                          <div className="absolute top-1 right-1 flex gap-1 opacity-0 group-hover:opacity-100 transition">
                            <button type="button" onClick={() => removeFileAt(i)}
                                    className="h-7 w-7 rounded-full bg-white/95 text-slate-700 shadow grid place-items-center" title="ลบรูป">✕</button>
                          </div>
                          <button type="button" onClick={() => makeCover(i)}
                                  className={["absolute bottom-1 left-1 text-[11px] font-medium px-2 py-0.5 rounded-full shadow",
                                    i===0 ? "bg-blue-600 text-white" : "bg-white/95 text-slate-700 hover:bg-white"].join(" ")}
            ...
            <div className="sticky bottom-0 bg-white/95 backdrop-blur border-t border-slate-100 rounded-full">
              <div className="px-4 sm:px-6 py-3 flex items-center justify-end gap-3 rounded-full">
                ...
                <button onClick={submit} disabled={saving || !valid}
                        className={["rounded-full px-6 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-blue-900 to-blue-700 shadow",
                          (saving || !valid) ? "opacity-60 cursor-not-allowed" : "hover:shadow-md active:scale-95"].join(" ")}>
...
function EmptyState({ onAdd }) {
  return (
    <div className="rounded-2xl border border-dashed border-slate-300 bg-white/70 p-10 text-center">
      ...
      <p className="text-slate-600 text-sm mt-1">เริ่มแจ้งพบของชิ้นแรกของคุณได้เลย</p>
      <button onClick={onAdd} className="mt-4 rounded-full px-5 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-blue-900 to-blue-700 shadow hover:shadow-md">
        + แจ้งพบของ
      </button>
    </div>
  );
...
function FoundCard({ item }) {
  ...
  const reporter =
    item.owner?.firstName || item.reporter?.firstName
      ? `${(item.owner?.firstName || item.reporter?.firstName) ?? ""} ${(item.owner?.lastName || item.reporter?.lastName) ?? ""}`.trim()
    ...
    <div className="group rounded-2xl overflow-hidden border border-slate-100 bg-white shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition">
      ...
      <div className="p-5">
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          <Link href={`/found/${item.id}`} className="text-sm text-blue-800 hover:underline whitespace-nowrap">ดูรายละเอียด</Link>
        </div>
      </div>
    </div>
  );
